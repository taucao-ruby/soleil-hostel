...
- Understanding how admin, hasRole, test_has_role_exact_match work
...
| `backend/tests/Unit/Models/UserRoleHelpersTest.php` | test_has_role_exact_match, test_role_is_cast_to_enum, test_factory_admin_state, test_is_user_returns_true_only_for_user_role, test_factory_moderator_state |
...
| `backend/app/Models/User.php` | hasRole, isUser |
...
1. `soleil-ai-review-engine_context({name: "admin"})` — see callers and callees
2. `soleil-ai-review-engine_query({query: "authorization"})` — find related execution flows
