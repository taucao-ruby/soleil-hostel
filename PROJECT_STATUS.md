...
**Current Branch:** `dev`
**Latest Commit:** `cbbfb10` — docs: refresh soleil index stamp — `dev` HEAD as of 2026-06-24. This reconciliation wave covers `edadbf5..cbbfb10` (77 commits); the prior reconciliation baseline was `edadbf5` (2026-06-14).

> **✅ Suite/gates RE-VERIFIED green on 2026-06-24 at `cbbfb10`** (+ Lane A working-tree changes: `momo_payments` money CHECK migration `2026_06_24_000001` + F-96 `CacheWarmupCommand` test-guard). Backend **1789 passed / 9 skipped / 5740 assertions** (sequential `phpunit` vs `soleil_test`, 11m56s); frontend **545 passed / 55 files** (`vitest`) + `tsc --noEmit` **0 errors**; **Pint 570 files PASS, PHPStan L5 0 errors, Psalm no errors (90.25% inferred)**. The full post-`edadbf5` wave (77 commits → `cbbfb10`: MoMo wallet payment, guzzle/psr7 + undici + form-data/vite security patches, E2E smoke-suite repair PR #16, Modern Archivist design system) is included in this run. Prior verified point: June 1, 2026 at `6d7b75b` (1697 backend / 488 frontend). Working tree is **uncommitted** — re-run after committing Lane A before the merge to `main`.
...
Gates (**re-verified green 2026-06-24 at `cbbfb10` + Lane A working tree — see the banner and Test Results Summary for current figures; the per-line June 1 figures below are retained as historical detail**; documentation layer May 8, 2026):
...
Open Findings: F-23 (MD lint, low), F-25 (CSRF path, low), F-26–F-62 (2026-03-20 audit — 36 open, F-48 resolved), F-63–F-66 (2026-04-05 audit — 4 findings), F-96 (local ParaTest `max_execution_time` stalls on Windows — low, 2026-06-21)
...
Payment bootstrap        ███████████████████░░░  85% ✅ Stripe Cashier (payment-hold + durable refund-event idempotency, TOCTOU eliminated) + MoMo wallet shipped end-to-end (in-app QR, IPN confirmation, momo_payments/momo_webhook_events idempotency); Stripe checkout UI still pending.
Documentation            █████████████████████░  99%
Deployment               ███████████████░░░░░░░  60%
─────────────────────────────────────────────
Total Progress           █████████████████████░  96%
...
> **Last verified suite run: 2026-06-24 at `cbbfb10`** (+ Lane A working-tree changes): backend **1789 passed / 9 skipped / 5740 assertions**; frontend **545 passed / 55 files**; `tsc --noEmit` **0 errors**; **Pint PASS (570 files), PHPStan L5 0 errors, Psalm no errors (90.25% inferred)**. Prior: June 1, 2026 at `6d7b75b` (1697 backend / 488 frontend / Pint 539 files); March 31, 2026 baseline (1047 backend / 261 frontend).

### Backend (PHPUnit/Pest)

```text
Jun 24 verified (cbbfb10 + Lane A): 1789 passed / 9 skipped / 5740 assertions / 716s  — sequential phpunit, soleil_test PostgreSQL
...
```text
Jun 24 verified (cbbfb10): 545 passed / 55 files / 100s  — vitest run; tsc --noEmit 0 errors
...
The never-green E2E booking smoke suite was repaired (PR #16, 7941727, 2026-06-18); pre-push now fails fast on a dead test DB and skips redundant backend tests (6913e2f). [NEEDS RUNTIME CONFIRMATION]
```
...
| ~~TL-03~~ | ~~Booking form submits `number_of_guests` and `special_requests` but backend does not validate or persist these fields~~ | ~~Medium~~ | ✅ Fixed May 25 — validated + persisted end-to-end (`1e32c8b` backend, `f6cc916` frontend, `7e2dcc5` contract docs) |
...
| **MoMo Wallet Payment (VN e-wallet)** | ✅ Done | Sandbox AIO v2 path shipped end-to-end (Jun 18–21): `MoMoService` adapter, `MoMoIpnHandler` + outcome enum + start DTO, `MoMoPaymentController` (`POST /v1/bookings/{booking}/momo/create` + public `POST /v1/payments/momo/ipn`), `momo_payments` authoritative order + `momo_webhook_events` idempotency ledger, services config + `.env.example`, `momo:simulate-ipn` dev command, `MoMoIpnTest`/`MoMoServiceTest`, frontend in-app QR (`createMoMoPayment`). IPN sign/verify symmetric + IPN route rate-limited 120/min. |
...
|--------|------|-------------|
| Jun 1–24, 2026 | **MoMo wallet payment shipped end-to-end** (sandbox AIO v2): `MoMoService` + `MoMoIpnHandler`/outcome enum/start DTO + `MoMoPaymentController`; `momo_payments`/`momo_webhook_events` tables; services config + `.env.example`; `momo:simulate-ipn` dev command; `MoMoIpnTest`/`MoMoServiceTest`; frontend in-app QR (`createMoMoPayment`). IPN sign/verify symmetric (`81421aa`) + public IPN route rate-limited (`d091910`). **Security:** guzzle/psr7 patched (`8334952`), undici→`^7.28.0` GHSA-vmh5-mc38-953g (`a772b67`), form-data CRLF + vite fs.deny (`13cae84`). **Test infra:** never-green E2E booking smoke suite repaired (PR #16, `7941727`); pre-push fails fast on dead test DB (`6913e2f`). **Frontend:** Modern Archivist design system (`f98fbe2`). | [NEEDS RUNTIME CONFIRMATION] at `cbbfb10` |
...
**June 1–24, 2026 (post-`edadbf5` → HEAD `cbbfb10`, 77 commits).** MoMo wallet payment (Vietnamese e-wallet) shipped end-to-end alongside Stripe: `MoMoService` gateway adapter, `MoMoIpnHandler` + `MoMoIpnOutcome` + `MoMoPaymentStartResult`, `MoMoPaymentController` exposing authenticated `POST /v1/bookings/{booking}/momo/create` and the public server→server `POST /v1/payments/momo/ipn`, two new tables (`momo_payments` — authoritative order record with a pinned `expected_amount`; `momo_webhook_events` — `(order_id, trans_id)`-UNIQUE idempotency ledger), a `config/services.php` `momo` block + `.env.example` sandbox keys, and the dev-only `momo:simulate-ipn` command. IPN sign/verify was made symmetric so valid IPNs are accepted (`81421aa`) and the public IPN route is rate-limited 120/min (`d091910`); the frontend renders an in-app MoMo QR in the booking flow (`createMoMoPayment`). Security: guzzle/psr7 patched, undici→`^7.28.0` (GHSA-vmh5-mc38-953g), form-data CRLF + vite `fs.deny` advisories patched. Test infra: the never-green E2E booking smoke suite was repaired (PR #16) and pre-push now fails fast on a dead test DB. Frontend: the Modern Archivist design system landed. New finding F-96 (local ParaTest `max_execution_time` stalls on Windows — low). **Runtime gates (tests, Pint, PHPStan, Psalm, tsc/vitest) require re-verification at `cbbfb10` before the next merge to `main`.** [NEEDS RUNTIME CONFIRMATION]

**May 9–31, 2026 (post-`6372d7f` → HEAD `b7d9d28`, 126 commits).** Booking-domain hardening dominated this wave: BL-1..BL-7 invariant fixes (restore empty-overlap race, refund-state overlap, constraint-first webhook idempotency, queued delivery, location_id three-layer guard, idempotency contract, confused-deputy cancellation); the A-1 mass-assignment defense (`Booking::$fillable` shrunk to user input — `d67b13f`); a durable Stripe webhook reaper with fail-closed signature verification; the `PaymentPolicy`×`PaymentStatus` state machine + `RefundStatus` projection (`f25273c`/`fcd323f`); refund correctness PAY-01/03/04 + SH-01/02/03 (idempotent unification, date immutability, ledger-on-all-paths); hostel-local timezone enforcement (`1b92d40`/`67d8302`); the room readiness endpoint + `RoomPolicy::updateReadiness` (moderator+); AI-harness RBAC moved from role-strings to Gate/Policy (`31eeeed`); `number_of_guests`/`special_requests` now persisted end-to-end (closes TL-03); a production runtime config-assertion pre-traffic gate; and rate-limit/CSP/CSRF + Symfony CVE hardening. Canonical + booking-domain docs were kept in sync (`14a9c67`/`2315188`). **Runtime gates (tests, Pint, PHPStan, Psalm) require re-verification at `b7d9d28` before the next merge to `main`.**
