...
| F-02 | `docs/README.md:162` | Tech Stack table says "Laravel 11 + PHP 8.3" but `composer.json` requires `laravel/framework:^12.0` and `php:^8.2`. | Low | Update to "Laravel 12 + PHP 8.2+". | **Fixed** (confirmed audit v5, 2026-02-23) |
| F-03 | `docs/README.md:155` | Project Status table says "142 frontend unit tests" but COMPACT.md reports 145 tests (13 suites) as of 2026-02-21. | Low | Update test count to 145. | **Partially fixed** — removed from README.md, residual in DEVELOPMENT_HOOKS.md:23 |
...
| F-15 | `backend/.env.test` | Tracked in git with `DB_CONNECTION=mysql` — contradicts PostgreSQL requirement. Contains placeholder credentials. | Medium | Remove from git or update to PostgreSQL config. | Open |
...
| F-17 | `backend/.env.testing:3` | Committed `APP_KEY=base64:...` — test-only but committed encryption keys are a hygiene issue. | Low | Set `APP_KEY=` (empty) and let CI generate it. | Open |
| F-18 | `frontend/src` (multiple files) | 8 TODO markers including `console.log` in SearchCard.tsx production code. | Low | Remove console.log; track TODOs in backlog. | Open |
