...
| A14 | `POST /api/v1/ai/proposals/{hash}/decide` | DENIED-EXPLICIT (401) | ALLOWED-OWN-PROPOSAL-ONLY | ALLOWED-OWN-PROPOSAL-ONLY | ALLOWED-OWN-PROPOSAL-ONLY | Route `check_token_valid` + `verified` + `throttle:5,1` + `ai_harness_enabled` (v1_ai.php:62-66) + Controller `proposer_user_id` check (ProposalConfirmationController.php:55-62) | AUTH-REQUIRED + FEATURE-GATED + OWNERSHIP-BOUND | YES (cache envelope proposer-binding + service-layer ownership) | CONFIRMED-V1 | AI-HARNESS-01 + F-06 (2026-04-18) |
...
- **Proposal decide (`A14`) is proposer-bound (F-06, 2026-04-18)**: the cache envelope carries `proposer_user_id`; `decide()` 404s when the authenticated user differs. Cancellation flow additionally enforces `CancellationService::validateCancellation` ownership check as a last-line service-layer gate. See `ARCHITECTURE_FACTS.md` §Proposer-Binding Invariant.
