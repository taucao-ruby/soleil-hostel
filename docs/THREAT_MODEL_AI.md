...
**Scope**: AI harness Phases 1–4 (F-06 proposer-binding remediation folded in 2026-04-18)
...
| T-13| Proposal ownership bypass       | Phase 4    | MEDIUM     | HIGH     | Proposer-binding (F-06, 2026-04-18): cache envelope carries `proposer_user_id`; `decide()` 404s on absence or mismatch. Hash entropy (64-char SHA-256) + per-user `throttle:5,1` + `Cache::forget()` post-decide provide layered mitigation. | Mitigated |
...
- **Controls**:
  1. **Proposer-binding** (F-06, 2026-04-18 — `ProposalConfirmationController.php:55-62`): cache envelope carries `proposer_user_id`; decide() 404s on absence or mismatch. Legacy cache entries without the field are also treated as unbound → 404.
...
- **Residual risk**: None after F-06. Proposer-binding is authoritative at the controller; service-layer ownership check catches any future regression at alternate callers.
