...
- Entry type: **docs-sync reconciliation** logging code that already landed on `dev` across `f5ffa02..edadbf5` (15 commits). Gates below are quoted from the authoring commits / FINDINGS_BACKLOG — **not** re-run in this docs pass; treat the runtime figures as [NEEDS RUNTIME CONFIRMATION] at the current HEAD.
- Change: **Currency → VND (was defaulting to USD)** — two backend fixes merged via `6e532b6` (`feature/payment-currency-vnd`). (1) `0307e95` adds `backend/config/cashier.php` defining `'currency' => env('CASHIER_CURRENCY', 'vnd')`; it loads before Laravel Cashier's `mergeConfigFrom`, so the vendor default `'usd'` no longer wins when `CASHIER_CURRENCY` is unset (the in-code `config('cashier.currency', 'vnd')` fallback never fired because the merged key is always present). This default flows into `bookings.payment_currency` (column default baked at migrate time + `CreateBookingService`) and Cashier's Stripe charge currency. (2) `f703452` corrects `BookingResource::amount_formatted`/`refund_amount_formatted` and `BookingController::buildCancellationMessage`, which formatted whole-VND amounts as USD cents (`'$'.number_format($amount/100, 2)` → e.g. `480000` VND shown as `$4,800.00`). Now rendered as whole VND with `vi-VN` grouping + `₫` (`number_format((int) $amount, 0, ',', '.').'₫'`) to match the frontend `formatVND` contract; backend contract tests assert presence only, so the response shape is unchanged.
- Change: **Admin trashed-bookings route + page (F-92 → Fixed)** — `7f7fd3b` (`feature/admin-trashed-bookings-route`, merged `862fcd9`) adds a static `bookings/trashed` route in `frontend/src/app/router.tsx` declared **before** the dynamic `bookings/:id` and a new `TrashedBookings` page consuming `GET /api/v1/admin/bookings/trashed`. Previously `/admin/bookings/trashed` fell through to `bookings/:id` and rendered `BookingDetailPage` with `id="trashed"`. The page is viewable by moderator+admin (inherits the admin section's `minRole=moderator`, matching the backend `role:moderator` gate); restore/force-delete are gated behind `isAdmin` plus backend enforcement. Closes the F-92 UX gap (no RBAC bypass existed — backend always gated the data).
- Change: **Trashed-view RBAC reconciliation (PERMISSION_MATRIX)** — `37e4120` corrects Table F (derived UI summary) so the trashed-bookings **view** reads moderator ✅ read-only / admin ✅ full (was admin-only), matching the authoritative Table A row A8 (`GET /v1/admin/bookings/trashed` enforced by `role:moderator`) and Table E. `952b38a` aligns Table B `booking:view-trashed` moderator cell `DENIED-SIDE-EFFECT → ROUTE-ACCESSIBLE` to match the `route:moderator` + view-all gate. `booking:restore`/`booking:force-delete` remain admin-only (A10/A11). This reconciliation also corrected the original F-92 premise (which had assumed the trashed view was admin-only).
- Change: **Moderator read-only trashed tab in `AdminDashboard` (`cfa0673`)** — the embedded `AdminDashboard` "Đã xóa" tab + fetch were gated behind `isAdmin`, hiding a view the backend allows moderators (A8). Introduced `canViewTrashed = isAdmin || role === 'moderator'`; split the bundled effect into a moderator+ trashed effect and an admin-only contacts effect; restore/force-delete buttons stay admin-only. Contact messages remain admin-only (A16/A17). Mirrors the dedicated `TrashedBookings` page.
- Change: **Nights accessor sign fix (F-93 → Fixed)** — `ee2f6a8` (`feature/booking-nights-sign-fix`, merged `1541070`) reorders `Booking::getNightsAttribute()` to `check_in->diffInDays(check_out)` (earliest-first). Carbon 3 made `diffInDays()` signed (Carbon 2 returned the absolute value), so the prior `check_out->diffInDays(check_in)` surfaced negative nights (e.g. `-2`) in `BookingResource`'s display-only `nights` field. Pricing was never affected (`CreateBookingService`/`Room` already order earliest-first). `BookingApiContractTest::test_show_nights_field_is_present_and_is_integer` tightened from integer-only to `assertSame(3, …)` to guard the Carbon 2→3 sign flip.
- Change: **Docs/infra housekeeping** — `dd348ac` logged F-92 (then Open) and F-93 (Fixed) into FINDINGS_BACKLOG; `e723443` gitignores RBAC preview verification artifacts; `1999729` then `edadbf5` refreshed the soleil-ai-review-engine index stamp in CLAUDE.md/AGENTS.md (`7329`/`21609` → final **7345 symbols / 21657 relationships**, 300 flows). Only the final `7345/21657` stamp survives at HEAD.
- Merge note: `4d0c532` ("Merge origin/dev") also brought origin/dev's **2026-06-13** work onto `dev`: P2-8 crypto-shredding + P2-9 retention/partitioning **design docs** (`e17cc8a`, `docs/design/p2-8-*.md` + `p2-9-*.md`), the `GHSA-gv7w-rqvm-qjhr` esbuild advisory allowlist with 90-day expiry (`4be3fd1`, `frontend/.audit-exceptions.json`), intermediate soleil index stamps (`adbcc32`/`312ad1e`), a Pint quote-style fix in `LedgerImmutabilityTriggerTest` (`f915202`), and e2e flake-ledger CI chores (`[skip ci]`). These are recorded here for range completeness; the P2-8/P2-9 design docs do not yet have a dedicated narrative entry (see Remaining Drift in the sync report).
- Scope: docs-sync pass touched only ledger/governance docs (WORKLOG, FINDINGS_BACKLOG F-92, PROJECT_STATUS, COMPACT §1, DB_FACTS currency alias, `.agent/rules/*` `last-verified`). The code/test/config changes above were authored and gated in their own feature branches; no `backend/`, `frontend/`, `.github/`, or `docker-compose*` file was modified in this docs pass.
...
- Change: **P1-6 — money-drift reconciliation (F-85)** — `reconciliation_refund_drift` view (`2026_06_12_000003`, pgsql-only; facets `refund_sum`, `refund_exceeds_amount`, `deposit_lifecycle`, `deposit_refund_exceeds_deposit`; tolerance from new `booking.reconciliation.drift_tolerance_cents` baked in at migrate time) + read-only `reconciliation:check-drift` command (always exits 0, per-row `reconciliation_drift.row_detected` warnings + run-level `reconciliation_drift.drift_detected` gauge following the webhook-reaper log-metric pattern) + scheduler entry dailyAt 04:10. Spec correction grounded in schema: the 2026_05_18 `stripe_webhook_events` fields are failure-context metadata, not money — refund sums read `stripe_refund_events.amount_refunded`; deposit lifecycle is `none/collected/applied/refunded/partial_refund/forfeited` (not hold/capture/release).
- Change: **P1-7 — decide() DB-authoritative + write ordering (F-86)** — gap [CONFIRMED] (was [INFERRED]): `Cache::put` preceded `persistDurableProposal` in `AiOrchestrationService::extractAndValidateProposals`, and `decide()` 404'd on cache miss without consulting `ai_proposals`. Fix: cold-cache fallback (`envelopeFromDurableRow`: proposer binding/consumed-state/expiry as SQL filters; `handleDecline` marks the durable row to close the replay window) + `storeProposal()` (durable row first, cache populate under `DB::afterCommit`). Tests: `ProposalColdCacheTest` (7), `ProposalStoreOrderingTest` (3, KeyWritten-probe ordering + read-through coherence + in-transaction invisibility); anti-tautology mutation check: legacy cache-first ordering fails 2/3 ordering tests.
- Verification (from `backend/`): `php artisan test tests/Feature/Reconciliation/ReconciliationDriftTest.php` → 10 passed/28 assertions (incl. exit-0-with-drift + idempotency); AiHarness suite → 178 passed/580 assertions; full `php artisan test --parallel` → 1775 passed/5693 assertions (9 skipped, pre-existing); `vendor/bin/pint --test` PASS (9 files); `vendor/bin/phpstan analyse` → no errors. soleil impact run pre-edit (`decide` LOW; `extractAndValidateProposals` flagged CRITICAL — single same-class d=1 caller, call contract unchanged, suite-verified); `detect_changes` "critical" cross-checked against raw diff (known line-shift over-report; actual symbols: `decide`, `handleDecline`, `extractAndValidateProposals` + 2 new methods).
- Merge: `feature/p1-6-p1-7-reconciliation-cold-cache` → dev `f5ffa02` (--no-ff, 3 commits: `89cd71c` feat / `efb3724` fix / `214ed72` test), branch deleted. Push pending separate confirmation. F-85/F-86 marked Fixed in FINDINGS_BACKLOG.
