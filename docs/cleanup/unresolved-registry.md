...
|----|----------------|----------|------------|-------------|--------------------------|------------------------|-------------------|--------|
| UNRESOLVED-B3-1 | B3 | agents.md | schema_gap | GitNexus `gitnexus:start/end` markers removed from agents.md. If `npx gitnexus analyze` re-injects on files without markers, duplication returns. | claude.md, agents.md | Confirm whether GitNexus CLI re-injects into agents.md or only claude.md. If both, add ignore directive. | no | open |
