...
export default function vitePluginCspNonce() {
  return {
    ...
    transformIndexHtml: function (html) {
      ...
      html = html.replace(/<script([^>]*)>/g, function (match, attrs) {
        // Bỏ qua external scripts (có src attribute)
        if (attrs.includes('src=')) {
          return match
        }
        // Thêm nonce vào inline scripts
        return '<script nonce="{{ csp_nonce() }}"'.concat(attrs, '>')
      })
      // Pattern 2: <style> tags trong bundle
      html = html.replace(/<style([^>]*)>/g, function (match, attrs) {
        return '<style nonce="{{ csp_nonce() }}"'.concat(attrs, '>')
      })
