...
const warn = m => { process.stderr.write(`[lint-doc-pointers] WARN ${m}\n`); hardFail = true; };
...
function isExcluded(p) {
  if (p.startsWith('http') || p.startsWith('/api/') || p.includes('://')) return true;
...
for (const { rel, trim } of targets) {
  ...
  for (const { raw, kind } of paths) {
    const base = (kind === 'mdlink' && (raw.startsWith('./') || raw.startsWith('../')))
      ? fileDir
      : root;
...
note(`audited ${auditedFiles} files; ${totalChecked} total cited paths; ${totalMissing} missing`);
